...
switch ($path) {
    ...
    case '/api/payments/transactions':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            (new \Livriko\Controllers\PaymentController())->createTransaction();
            exit();
        }
        http_response_code(405);
        exit();
    ...
    case '/api/payments/status':
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            (new \Livriko\Controllers\PaymentController())->status();
            exit();
        }
        http_response_code(405);
        exit();
    ...
    case '/api/maps/route':
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            (new \Livriko\Controllers\MapsController())->route();
            exit();
        }
        http_response_code(405);
        exit();

    case '/api/webhooks/mtn':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            require __DIR__ . '/webhooks/mtn.php';
            exit();
        }
        http_response_code(405);
        exit();
