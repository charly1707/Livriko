...
class ProductController
{
    ...
    public function createProduct(): void
    {
        try {
            $ownerId = $this->getAuthenticatedUserId();
            if ($ownerId === null) {
                $this->respondJson(['success' => false, 'message' => 'Non authentifié.'], 401);
                return;
            }

            $restaurant = $this->restaurantModel->findByOwnerId($ownerId);
            if (!$restaurant) {
                $this->respondJson(['success' => false, 'message' => 'Restaurant introuvable.'], 404);
                return;
            }
            ...
            if (empty($data['nom']) || $data['prix'] <= 0) {
                $this->respondJson(['success' => false, 'message' => 'Nom du produit et prix valides requis.'], 400);
                return;
            }
            ...
            if ($product === null) {
                $this->respondJson(['success' => false, 'message' => 'Le produit n’a pas été enregistré en base.'], 500);
                return;
            }

            $this->respondJson(['success' => true, 'message' => 'Produit ajouté avec succès.', 'product' => array_merge($product, [
                'store_id' => 'store-' . $ownerId,
                'store_name' => $restaurant['nom'],
            ])], 201);
        } catch (\Throwable $exception) {
            error_log('Product creation error: ' . $exception->getMessage());
            $this->respondJson(['success' => false, 'message' => 'Une erreur interne est survenue pendant la création du produit.'], 500);
        }
    }
