...
class PaymentController
{
    ...
    public function createTransaction(): void
    {
        ...
        try {
            ...
            $stmt->execute([$orderId, $userId, $providerName, $result['providerTransactionId'], $idempotencyKey, $phone, $order['total'], $config['payment']['currency'], $result['status'], json_encode($result['raw'])]);
            $this->json(['success' => true, 'transaction' => $this->publicTransaction(['id' => $this->db->lastInsertId(), 'provider' => $providerName, 'provider_transaction_id' => $reference, 'amount' => $order['total'], 'currency' => $config['payment']['currency'], 'status' => 'pending'])], 201);
    ...
    {
        $userId = $this->userId();
        if ($userId === null) { $this->json(['success' => false, 'message' => 'Non authentifié.'], 401); return; }
        ...
        if (!$transaction) { $this->json(['success' => false, 'message' => 'Transaction introuvable.'], 404); return; }
        $this->json(['success' => true, 'transaction' => $this->publicTransaction($transaction)]);
    }

    private function publicTransaction(array $transaction): array
    {
        return ['id' => (int)$transaction['id'], 'provider' => $transaction['provider'], 'providerTransactionId' => $transaction['provider_transaction_id'] ?? null, 'amount' => (float)$transaction['amount'], 'currency' => $transaction['currency'], 'status' => $transaction['status']];
    }
