...
final class MapsService
{
    ...
    public function route(float $fromLat, float $fromLng, float $toLat, float $toLng): array
    {
        ...
        if (!empty($this->config['api_key'])) $headers[] = 'Authorization: Bearer ' . $this->config['api_key'];
        $curl = curl_init($url);
        ...
        curl_close($curl);
        if ($raw === false || $error !== '' || $status < 200 || $status >= 300) throw new RuntimeException('API GPS indisponible.');
        $body = json_decode($raw, true);
        if (!is_array($body)) throw new RuntimeException('Réponse cartographique invalide.');
        return $body;
    }
}
