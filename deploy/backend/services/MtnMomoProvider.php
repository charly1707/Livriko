...
final class MtnMomoProvider implements PaymentProvider
{
    ...
    private function requestToken(): string
    {
        ...
        if (!$token) throw new RuntimeException('Le fournisseur MTN MoMo n’a pas fourni de jeton.');
        return $token;
    }
    ...
    private function request(string $method, string $url, array $headers, ?array $payload = null): array
    {
        ...
        curl_close($curl);
        if ($raw === false || $error !== '') throw new RuntimeException('Fournisseur de paiement indisponible.');
