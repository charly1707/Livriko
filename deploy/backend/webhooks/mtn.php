...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') webhookResponse(['success' => false, 'message' => 'Méthode non autorisée.'], 405);

$config = require __DIR__ . '/../config/production.php';
$raw = file_get_contents('php://input');
$payload = json_decode($raw, true);
if (!is_array($payload)) webhookResponse(['success' => false, 'message' => 'Payload invalide.'], 400);
...
if ($eventId === '') webhookResponse(['success' => false, 'message' => 'Identifiant événement requis.'], 400);
...
try {
    $db->beginTransaction();
    $event = $db->prepare('INSERT INTO webhook_events (provider, event_id, signature_valid, payload) VALUES (?, ?, 1, ?)');
    try {
        $event->execute(['mtn_momo', $eventId, $raw]);
    ...
    $transaction = $db->prepare('SELECT id, order_id FROM payment_transactions WHERE provider = ? AND provider_transaction_id = ? LIMIT 1');
    $transaction->execute(['mtn_momo', $providerTransactionId]);
    $payment = $transaction->fetch();
    if ($payment) {
        $db->prepare('UPDATE payment_transactions SET status = ?, provider_payload = ?, failure_reason = CASE WHEN ? IN (\'failed\', \'cancelled\') THEN ? ELSE failure_reason END WHERE id = ?')->execute([$status, $raw, $status, $payload['reason'] ?? null, $payment['id']]);
        if ($status === 'successful') {
            $db->prepare('UPDATE commandes SET statut_paiement = \'paid\' WHERE id = ? AND statut_paiement <> \'paid\'')->execute([$payment['order_id']]);
        }
    }
